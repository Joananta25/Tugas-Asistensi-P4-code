...
void play(Character* character) {
    if (strcmp(character->type, "pet") == 0) {
        character->loves += 10;
        character->energy -= 5;
        printf("%s sedang bermain! Loves sekarang: %d, Energy sekarang: %d\n", character->name, character->loves, character->energy);
    } else {
        printf("%s bukan hewan! Perintah PLAY hanya bisa digunakan pada hewan.\n", character->name);
    }
}
...
int main() {
    ...
    printf("Gunakan perintah 'PLAY <NAMA>' untuk bermain dengan hewan.\n");
